...
- **In-app subtitle editor** — open from the transcript preview footer or the pencil icon on any successful history row. Edit segment text directly (timestamps stay read-only to prevent accidental timeline damage), with Cmd/Ctrl+F find-and-replace, IME-aware keyboard handling, and a tmp-dir backup of every original before overwrite. A single Save rewrites exactly the formats you have enabled in **Settings → Subtitle output** (`.srt` / `.vtt` / `.txt` / `.json`) — JSON is patched field-by-field so Whisper's `logprob` / `id` / `duration` metadata survives intact
