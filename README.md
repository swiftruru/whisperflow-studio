...
- **In-app subtitle editor** — open from the transcript preview footer or the pencil icon on any successful history row. Edit segment text directly (timestamps stay read-only to prevent accidental timeline damage), with full undo / redo history that survives saves (⌘Z / ⌘⇧Z keyboard shortcuts, 200-entry stack), Cmd/Ctrl+F find-and-replace with case-sensitive toggle and match counter, IME-aware keyboard handling so 注音/Pinyin/Kana typing works correctly, and a tmp-dir backup of every original before overwrite. A single Save rewrites exactly the formats you have enabled in **Settings → Subtitle output** (`.srt` / `.vtt` / `.txt` / `.json`) — JSON is patched field-by-field so Whisper's `logprob` / `id` / `duration` metadata survives intact
...
- **Transcription history** — last 10 transcribed files (✓ / ✗) persisted across sessions. Auto-prunes stale rows (media file **and** transcript output both gone) at boot, after each new entry, and on a per-tap **Refresh** button in the history header — so you never see rows whose files have been deleted. A conservative policy: as long as either the source media or the subtitle is still on disk the row stays
